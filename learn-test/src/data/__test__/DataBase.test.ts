...
describe("DataBase test suit data base store data as an array withe type T inisde it, also need have id on the T", () => {
  ...
  it("Database insert method insert T data type to array and return teh id", async () => {
    ...
    const mockElemen = (suit as any).elements;
    expect(actual).toBe(fakeId);
    expect(mockElemen).toEqual([{ id: fakeId }]);
  });
  it("Database insert will pust the data type T to the pivate element", async () => {
    await suit.insert(someObject);
    const mockElemen = (suit as any).elements;
    expect(mockElemen).toEqual([someObject]);
  });
  it("Database getBy will looking on the stored data element the match data withe propery we put ", async () => {
    const id = await suit.insert(someObject);
    const actial = await suit.getBy("id", id);
    expect(actial).toEqual(someObject);
  });
  it("Database getBy will looking on the stored data element the match data withe propery we put", async () => {
    const id = await suit.insert(someObject);
  ...
  it("Database findAllBy will fond all data match the property we put", async () => {
    ...
    expect(mockElemen).toEqual(expect.arrayContaining(actual));
  });
  ...
  it("Database getAllElements will returns all alament we store on Database", async () => {
    await suit.insert(someObject);
    await suit.insert(someObject2);
